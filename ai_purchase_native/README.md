# AI Purchase PDF - Native Odoo AI (Odoo 19) — v19.0.2.1.1
...
## Automatic master-data creation (v19.0.2.1.1)
...
## Gemini timeout hardening (v19.0.2.1.1)

- Fixes the `UnboundLocalError` that could hide the real Gemini error after a timeout.
- Prefers a Gemini Flash model exposed by Odoo for lower-latency synchronous extraction.
- For text-based PDFs, Odoo extracts embedded text locally and sends that smaller text payload to the configured native Gemini provider. Gemini still performs the semantic purchase extraction.
- For scanned/image-only PDFs, the module falls back to Odoo AI PDF file input.
- Text-mode timeouts are retried once and all remaining timeout errors are shown as a clean user-facing message instead of an RPC traceback.
