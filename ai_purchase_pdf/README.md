...
- Fixed the Odoo 19 search-view definition for `ai.purchase.document`.
- Removed legacy `expand` and `string` attributes from the search-view `<group>` node, which are rejected by the Odoo 19 search view Relax NG schema.
