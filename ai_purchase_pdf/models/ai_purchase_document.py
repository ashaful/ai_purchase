...
class AiPurchaseDocument(models.Model):
    ...
    ai_raw_json = fields.Text(string='AI Raw JSON', readonly=True, groups='base.group_system')
    ...
    def action_process_pdf(self):
        ...
        try:
            ...
            self.state = 'review'
            self.message_post(body=_('AI extraction completed. Some vendor/product matches require human review.'))
            return self._notify(
                _('Review Required'),
                _('AI extraction is complete. Please review the unmatched or uncertain vendor/product fields.'),
                'warning',
                True,
            )
        except Exception as exc:
            ...
            self.message_post(body=_('AI processing failed: %s') % message)
            return self._notify(_('AI Processing Failed'), message, 'danger', True)
